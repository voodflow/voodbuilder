...
/**
 * Security checks for GrapesJS block catalog endpoints.
 */
final class GrapesJsBlocksControllerSecurityTest extends TestCase
{
    use RefreshDatabase;
    ...
    public function test_blocks_catalog_requires_page_builder_access(): void
    {
        ...
        $this->actingAs($user)
            ->getJson(route('voodbuilder.grapesjs.blocks'))
            ->assertForbidden();
    }
    ...
    {
        $this->getJson(route('voodbuilder.grapesjs.blocks'))
            ->assertUnauthorized();
    }
}
