# Linea guida: authoring blocchi GrapesJS (VoodBuilder)
...
Contesto tipico del problema: hero a strati (media assoluto + shade + contenuto `z-10`). Se il markup non è pensato per GrapesJS, l’immagine non si clicca, i pulsanti finiscono *sopra* la section e il menu contestuale “lampeggia”.
...
>
  ...
  <div
    class="voodbuilder-gjs-container relative z-10 …"
...
> Stai authorando blocchi GrapesJS per VoodBuilder. Ogni section deve avere: (1) media layer non-droppable con cover CSS stabile `.voodbuilder-hero-media*`, (2) shade `pointer-events-none`, (3) content dropzone esplicita `data-voodbuilder-dropzone="content|copy|actions"` dove si droppano Button/testo della libreria senza uscire dalla section, (4) trait Content-panel per cambiare l’immagine hero senza cacciare l’img sotto lo shade, (5) token `vp-*`, (6) editor e render pubblico equivalenti. Segui `packages/voodflow/voodbuilder/docs/GRAPESJS_BLOCK_AUTHORING.md`. Non nascondere overlay per editare media. Non annidare section. Aggiorna `@source` in `section-utilities.css` e testa drop Button + cambio img da Content + preview pubblica senza letterbox.
