...
final class EditorPastedComponentNormalizer
{
    ...
    public static function normalize(string $input): array
    {
        ...
        $html = EditorImportedTailwindSupport::prepareHtml($html);
        ...
        $html = EditorHtmlSanitizer::sanitize(trim($html));
    ...
    {
        $compiled = EditorComponentTailwindCompiler::compile($html, $scope);

        if ($compiled === null) {
            $compiled = $scope === 'page'
                ? EditorImportedTailwindCssBuilder::buildForPage($html)
    ...
    /**
     * CSS for page-level Editor styles (non-component utilities).
     */
     ...
     * Also catches orphan `animation: var(--animate-*)` utilities without the matching
     * custom property (overrides theme.css and leaves published pages static).
     */
    public static function pageCssMissingThemeVariables(string $css): bool
    {
        ...
        if (str_contains($css, 'var(--spacing)')
            && preg_match('/:root[^{]*\{[^}]*--spacing\s*:/s', $css) !== 1) {
            return true;
        }

        return self::pageCssHasOrphanAnimateVariables($css);
    }

    /**
     * Page CSS that redefines .animate-* with var(--animate-*) but omits the token
     * beats theme.css and disables motion on the published frontend.
     */
    public static function pageCssHasOrphanAnimateVariables(string $css): bool
    {
        if (! preg_match_all('/var\(--(animate-[a-z0-9-]+)\)/i', $css, $matches)) {
            return false;
        }
    ...
    public static function isTailwindUtilityClassName(string $className): bool
    {
        ...
        // Include animate-* (tailwindcss-animated + core spin/pulse/bounce) so Style Manager
        // animation utilities are regenerated by page JIT and not kept as orphan manual CSS.
    ...
    /**
     * Editor Style Manager rules (#element-id) — excludes Tailwind utility bundles from getCss().
     */
    ...
    public static function stripComponentRuntimeBaseStyles(string $css, string $html): string
    {
        ...
        foreach (explode("\n", EditorImportedTailwindCssBuilder::baseStyles()) as $line) {
            $line = trim($line);
...
:where(.voodbuilder-editor-component-instance, .voodbuilder-component-rendered, .VPRichPage) :where(.voodbuilder-pasted-component) .text-white {
    color: var(--color-white);
}
:where(.voodbuilder-editor-component-instance, .voodbuilder-component-rendered, .VPRichPage) :where(.voodbuilder-pasted-component) .text-primary {
    color: var(--color-primary);
}
:where(.voodbuilder-editor-component-instance, .voodbuilder-component-rendered, .VPRichPage) :where(.voodbuilder-pasted-component) .text-foreground {
    color: var(--color-foreground);
}
:where(.voodbuilder-editor-component-instance, .voodbuilder-component-rendered, .VPRichPage) :where(.voodbuilder-pasted-component) .text-foreground-inverse {
    color: var(--color-foreground-inverse);
}
:where(.voodbuilder-editor-component-instance, .voodbuilder-component-rendered, .VPRichPage) :where(.voodbuilder-pasted-component) .text-inverse {
    color: var(--color-inverse);
}
.dark :where(.voodbuilder-editor-component-instance, .voodbuilder-component-rendered, .VPRichPage) :where(.voodbuilder-pasted-component) .dark\:text-neutral-900 {
    color: var(--color-neutral-900);
}
:where(.voodbuilder-editor-component-instance, .voodbuilder-component-rendered, .VPRichPage) :where(.voodbuilder-pasted-component) .bg-card {
    background-color: var(--color-vp-bg-elv);
}
:where(.voodbuilder-editor-component-instance, .voodbuilder-component-rendered, .VPRichPage) :where(.voodbuilder-pasted-component) .bg-layer {
    background-color: var(--color-vp-bg-elv);
}
:where(.voodbuilder-editor-component-instance, .voodbuilder-component-rendered, .VPRichPage) :where(.voodbuilder-pasted-component) .bg-surface {
    background-color: var(--color-vp-bg-alt);
}
:where(.voodbuilder-editor-component-instance, .voodbuilder-component-rendered, .VPRichPage) :where(.voodbuilder-pasted-component) .bg-surface-1 {
    background-color: var(--color-vp-bg-alt);
}
CSS;
    }
