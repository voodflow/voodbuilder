...
final class ComponentRuntimeBridge
{
    ...
    {
        if (! self::safeClassExists(EditorGlobalClassRenderer::class)) {
            return null;
        }

        try {
            $css = app(EditorGlobalClassRenderer::class)->css();
        } catch (\Throwable) {
            return null;
        }
    ...
    {
        if (! self::safeClassExists(EditorComponentCssRenderer::class)) {
            return null;
        }

        try {
            $css = app(EditorComponentCssRenderer::class)->cssForHtml($html);
        } catch (\Throwable) {
            return null;
        }
    ...
    {
        if (! self::safeClassExists(EditorComponentRenderer::class)) {
            return $html;
        }

        try {
            return app(EditorComponentRenderer::class)->render($html, $page);
        } catch (\Throwable) {
            return $html;
        }
    }
    ...
    {
        if (! self::safeClassExists(EditorComponentCssLibrarySync::class)) {
            return;
        }

        try {
            app(EditorComponentCssLibrarySync::class)->syncFromPageHtml($html);
        } catch (\Throwable) {
