...
final class EditorSlotHydrator
{
    ...
    public static function renderBrand(bool $preview = false, array $config = []): string
    {
        ...
        return view('voodbuilder::editor.blocks.partials.footer-brand', [
    ...
    {
        if ($menuSlug === 'social') {
            return view('voodbuilder::editor.blocks.partials.social-menu-list-wrapper', [
                'menuSlug' => $menuSlug,
                'preview' => $preview,
                'config' => $config,
        ...
        return view('voodbuilder::editor.blocks.partials.footer-menu-list-wrapper', [
            'menuSlug' => $menuSlug,
            'preview' => $preview,
        ])->render();
    }
    ...
    protected static function resolveConfigForElement(DOMElement $element, array $fallback = []): array
    {
        ...
        while ($current instanceof DOMElement) {
            if ($current->hasAttribute('data-voodbuilder-config')) {
                $decoded = EditorDynamicBlockAttributeNormalizer::decodeConfig(
