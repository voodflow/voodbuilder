...
final class VoodbuilderThemeTokenMigrator
{
    ...
    private static function migrateHtmlClassAttributes(string $html): string
    {
        if (! str_contains($html, 'data-voodbuilder-component')) {
            // Migrate per opening tag so we can honour data-voodbuilder-content-width
            // (toolbar measure) and avoid re-forcing layout max-w utilities.
            return preg_replace_callback(
                ...
                static function (array $matches): string {
                    ...
                    if ($attrs === '' || ! preg_match('/\bclass=(["\'])(.*?)\1/s', $attrs, $classMatch)) {
                        return $matches[0];
                    }

                    $respectContentWidth = self::openingTagHasContentWidthAttr($attrs);
                    $quote = $classMatch[1];
        ...
        foreach ($document->getElementsByTagName('*') as $element) {
            ...
            $respectContentWidth = self::elementHasContentWidthAttr($element);
            $element->setAttribute(
                'class',
                self::migrateClassList($element->getAttribute('class'), $respectContentWidth),
            );
        }
    ...
    private static function openingTagHasContentWidthAttr(string $attrs): bool
    {
        return preg_match(
            '/\bdata-voodbuilder-content-width=(["\'])(full|normal|custom)\1/',
            $attrs,
        ) === 1;
    }

    private static function elementHasContentWidthAttr(\DOMElement $element): bool
    {
        $mode = strtolower(trim($element->getAttribute('data-voodbuilder-content-width')));

        return in_array($mode, ['full', 'normal', 'custom'], true);
    }
    ...
    {
        return preg_replace_callback(
            '/var\(--color-(blue|green|indigo|yellow|red|purple|violet|pink|sky|emerald|amber|teal|rose|fuchsia|cyan|orange|lime|slate|gray|zinc|neutral|stone)-(\d+)\)(?!\s*,)/i',
            static function (array $matches): string {
    ...
    /**
     * @param  bool  $respectContentWidth  When true, do not force mx-auto /
     *                                     max-w-[var(--width-vp-layout)] — the
     *                                     data-voodbuilder-content-width toolbar owns measure.
     */
    public static function migrateClassList(string $classList, bool $respectContentWidth = false): string
    {
        $tokens = preg_split('/\s+/', trim($classList), -1, PREG_SPLIT_NO_EMPTY);
        ...
        $tokens = self::syncContainerTailwindUtilities($tokens, $respectContentWidth);
     ...
     * `voodbuilder-editor-container` (max-width + mx-auto) letterboxes the image
    ...
    private static function stripContainerLayoutFromHeroMedia(array $tokens): array
    {
        ...
        $deny = array_merge(
            ['voodbuilder-editor-container', 'container'],
    ...
    /**
     * Tailwind `.container` uses viewport breakpoints that break inside the Editor canvas
    ...
    private static function migrateContainerClass(array $tokens): array
    {
        ...
        $tokens = array_map(static function (string $token): string {
            while (str_contains($token, 'voodbuilder-editor-voodbuilder-editor-container')) {
                $token = str_replace('voodbuilder-editor-voodbuilder-editor-container', 'voodbuilder-editor-container', $token);
            }
        ...
        if (! in_array('voodbuilder-editor-container', $tokens, true)) {
            array_unshift($tokens, 'voodbuilder-editor-container');
        }
    ...
    /**
     * Keep layout when `voodbuilder-editor-container` is present: mirror its global CSS
     * as Tailwind utilities so per-block JIT compile also constrains width.
     *
     * When the author set data-voodbuilder-content-width, skip forcing layout
     * max-w / mx-auto (those fight full/normal/custom toolbar modes on save).
     *
     * @param  list<string>  $tokens
     * @return list<string>
     */
    private static function syncContainerTailwindUtilities(array $tokens, bool $respectContentWidth = false): array
    {
        if (! in_array('voodbuilder-editor-container', $tokens, true)) {
            return $tokens;
        }

        if ($respectContentWidth) {
            // Keep w-full for flex/grid children; drop layout-measure utilities the
            // content-width toolbar already stripped (or replaced with max-w-[80rem]).
            return array_values(array_filter(
                $tokens,
                static fn (string $token): bool => $token !== 'max-w-[var(--width-vp-layout)]',
            ));
        }
     ...
     * editors remove `voodbuilder-editor-container` from the code modal.
     *
    ...
    private static function ensureSectionBlockContainers(string $html): string
    {
        ...
        return preg_replace_callback(
            ...
            static function (array $matches): string {
                ...
                if (in_array('voodbuilder-editor-container', $tokens, true) || in_array('container', $tokens, true)) {
                    return $matches[0];
                }
    ...
    {
        if (! in_array('voodbuilder-editor-btn-primary', $tokens, true)) {
            return $tokens;
        }

        $tokens = array_values(array_filter(
            $tokens,
            static fn (string $token): bool => $token !== 'voodbuilder-editor-btn-primary',
        ));
    ...
    private static function ensureSectionClasses(string $classes): string
    {
        ...
        if (! in_array('voodbuilder-editor-section', $tokens, true)) {
            $tokens[] = 'voodbuilder-editor-section';
        }
    ...
    {
        $classList = null;
        $mode = isset($attrs['data-voodbuilder-content-width'])
            ? strtolower(trim((string) $attrs['data-voodbuilder-content-width']))
            : '';
        $respectContentWidth = in_array($mode, ['full', 'normal', 'custom'], true);

        if (isset($attrs['class']) && is_string($attrs['class'])) {
            $classList = self::migrateClassList($attrs['class'], $respectContentWidth);
    ...
    private static function styleRuleTargetsLegacyButton(array $styleRule): bool
    {
        ...
        foreach ($selectors as $selector) {
            if (is_string($selector) && str_contains($selector, 'voodbuilder-editor-btn-primary')) {
                return true;
            }
        }
    ...
    {
        return preg_replace(
            '/\.voodbuilder-editor-btn-primary(?::hover)?\s*\{[^}]*\}\s*/',
            '',
            $css,
        ) ?? $css;
    }
     ...
     * Do not strip Tailwind palette scales — decorative utilities need those definitions.
     */
     ...
     *
     * Older builds stripped --color-indigo/violet/… so ThemePalette brand bridges
     * could own those names. That made literal palette utilities (text-violet-400/10)
     * resolve to an undefined var and inherit page text color (white on dark pages).
     */
     ...
     * Intentionally a no-op.
     *
     * Palette utilities (indigo/violet/…) must stay literal. Theme-aware UI uses
     * vp-brand-* classes; remapping CSS vars here made decorative colors follow
     * the theme (same bug as compile-time rewriteLegacyPaletteUtilityColors).
     */
