...
export function expandLibraryCategories(editor, libraryId = editor.__voodbuilderActiveLibrary ?? 'blocks') {
...
export function collapseLibraryCategories(editor, libraryId = editor.__voodbuilderActiveLibrary ?? 'blocks') {
    // Selection mode needs visible blocks — keep categories open (and re-open after
    // BlockManager.render / library relocate which would otherwise collapse them).
    if (libraryId === 'templates' && editor?.__voodbuilderTemplateSelectionMode === true) {
        expandLibraryCategories(editor, 'templates');

        return;
    }

    if (libraryId === 'components' && editor?.__voodbuilderComponentSelectionMode === true) {
...
export function applyBlocksLibraryUi(editor, searchQuery = '') {
    ...
    editor.BlockManager.getCategories?.()?.each?.((category) => {
        ...
        categoryEl.classList.toggle('voodbuilder-editor-component-category', isComponentCategory);
        categoryEl.classList.toggle('voodbuilder-editor-page-template-category', isTemplateCategory);
...
export function readBlocksSearchQuery() {
    const shell = document.querySelector('.voodbuilder-editor-shell');

    return shell?.querySelector('.voodbuilder-editor-blocks-search')?.value?.trim() ?? '';
}
