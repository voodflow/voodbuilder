...
export function buildRichTextDynamicTagHtml({ bindingKey, sourceLabel, fieldLabel, fieldType }) {
    ...
    if (String(fieldType ?? 'text').toLowerCase() === 'url') {
        const classes = escapeHtml(['voodbuilder-editor-bound', 'vb-rich-text-dynamic', ...RICH_TEXT_LINK_CLASSES].join(' '));
    ...
    return `<span class="voodbuilder-editor-bound vb-rich-text-dynamic" contenteditable="false" data-voodbuilder-bind="${safeKey}" data-voodbuilder-hide-when-empty="1">${placeholder}</span>`;
}
