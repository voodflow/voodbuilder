/**
 * Editor toolbar and style-manager setup for Voodbuilder.
...
import { openGlobalTextTagsDialog } from './global-text-tags-dialog.js';
...
/**
 * Native Grapes Style Manager sectors are intentionally empty.
 * Styling is owned by the Tailwind utility panel (`style-tailwind-panel.js`)
 * plus the Animation sector — classes are the source of truth.
 * Kept as an exported constant so layout/chrome options stay compatible.
 */
...
function createToolButton({ id, icon, title, active = false, iconSet = 'lucide' }) {
    ...
    button.type = 'button';
    button.className = 'voodbuilder-editor-topbar-tool-btn';
...
function createToolGroup(children) {
    ...
    group.className = 'voodbuilder-editor-topbar-tool-group';
...
function createToolDivider() {
    ...
    divider.className = 'voodbuilder-editor-topbar-tool-divider';
...
function mountEditorTopbar(editor, mount, labels = {}, shellRoot = null) {
    ...
    root.className = 'voodbuilder-editor-topbar-tools-inner';
    ...
    const globalTagsButton = createToolButton({
        id: 'global-text-tags',
        icon: 'replace',
        iconSet: 'tabler',
        title: labels.globalTextTags ?? 'Text tags',
    });
    globalTagsButton.addEventListener('click', () => openGlobalTextTagsDialog(editor, labels));
    ...
    readout.className = 'voodbuilder-editor-topbar-device-readout';
...
function mountViewPageAction(actionsMount, viewPageUrl, labels = {}) {
    ...
    link.className = 'voodbuilder-editor-topbar__btn voodbuilder-editor-topbar__btn--ghost voodbuilder-editor-topbar__btn--icon';
...
/** Editor frame width/height transition (.gjs-frame-wrapper--anim). */
