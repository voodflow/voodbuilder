...
export function registerPageTemplatesUi(editor, options = {}) {
    ...
    modal.hidden = true;
    modal.innerHTML = `
        <div class="voodbuilder-editor-modal__backdrop" data-voodbuilder-page-templates-close></div>
        <div class="voodbuilder-editor-modal__panel" role="dialog" aria-modal="true">
            <header class="voodbuilder-editor-modal__head">
                <h2 class="voodbuilder-editor-modal__title">${labels.pageTemplatesTitle ?? 'Page templates'}</h2>
                <button type="button" class="voodbuilder-editor-modal__close" data-voodbuilder-page-templates-close aria-label="Close">×</button>
            </header>
            <div class="voodbuilder-editor-modal__body">
                <p class="voodbuilder-editor-hint">${labels.pageTemplatesHint ?? 'Save the current page or apply a saved layout.'}</p>
                <div class="voodbuilder-editor-dynamic-panel__actions">
                    <button type="button" class="voodbuilder-editor-btn voodbuilder-editor-btn--primary" data-voodbuilder-page-template-save>
                        ${labels.pageTemplatesSave ?? 'Save current page'}
                    </button>
                    <button type="button" class="voodbuilder-editor-btn voodbuilder-editor-btn--ghost" data-voodbuilder-page-template-import>
                        ${labels.pageTemplatesImport ?? 'Import bundle'}
                    </button>
                    <button type="button" class="voodbuilder-editor-btn voodbuilder-editor-btn--ghost" data-voodbuilder-page-template-import-url>
    ...
    async function loadTemplates() {
        listEl.innerHTML = `<p class="voodbuilder-editor-hint">${labels.pageTemplatesLoading ?? 'Loading…'}</p>`;
    ...
    function renderList(templates) {
        if (templates.length === 0) {
            listEl.innerHTML = `<p class="voodbuilder-editor-hint">${labels.pageTemplatesEmpty ?? 'No page templates yet.'}</p>`;
        ...
        for (const template of templates) {
            ...
            applyBtn.className = 'voodbuilder-editor-btn voodbuilder-editor-btn--primary';
            ...
            deleteBtn.className = 'voodbuilder-editor-btn voodbuilder-editor-btn--ghost';
    ...
    async function openModal() {
        ...
        try {
            ...
            listEl.innerHTML = `<p class="voodbuilder-editor-hint">${labels.pageTemplatesLoadError ?? 'Could not load page templates.'}</p>`;
        }
        ...
        try {
            ...
            if (catalogEl && pageTemplatesCatalogUrl) {
                ...
                catalogEl.innerHTML = `<p class="voodbuilder-editor-hint">${labels.pageTemplatesCatalogInstallError ?? 'Could not install that template.'}</p>`;
            }
        }
    }
    ...
    async function loadCatalog() {
        ...
        catalogEl.hidden = false;
        catalogEl.innerHTML = `<p class="voodbuilder-editor-hint">${labels.pageTemplatesLoading ?? 'Loading…'}</p>`;
        ...
        if (entries.length === 0) {
            catalogEl.innerHTML = `<p class="voodbuilder-editor-hint">${labels.pageTemplatesCatalogEmpty ?? 'No remote templates are available.'}</p>`;

            return;
        }

        catalogEl.innerHTML = `
            <h3 class="voodbuilder-editor-subtitle">${labels.pageTemplatesCatalogTitle ?? 'Template marketplace'}</h3>
        ...
        for (const entry of entries) {
            ...
            description.className = 'voodbuilder-editor-page-templates-catalog__description';
            ...
            installBtn.className = 'voodbuilder-editor-btn voodbuilder-editor-btn--primary';
