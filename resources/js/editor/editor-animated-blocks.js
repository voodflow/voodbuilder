...
function clearLogoScrollRepeatMarkers(track) {
    ...
    track.removeAttributes?.('data-voodbuilder-repeat-filter');
...
function syncAnimatedStats(component) {
    ...
    if (! root) {
        const container = component.find('.voodbuilder-editor-container')[0]
...
const BLOCKS = [
    {
        ...
        content: {
            ...
            'data-vb-anim-delay': 0,
            components: `
                <div class="voodbuilder-editor-container px-5 py-20 text-center">
...
export function configureAnimatedCanvas(editor) {
    ...
    let lastCounterPlayAt = 0;

    const settlePreview = ({ forceCounters = false } = {}) => {
        try {
            ...
            const now = Date.now();
            // Chrome remounts recreate counter DOM at the final label — replay, but
            // throttle so frequent site-chrome-updated events do not loop forever.
            const playCounters = forceCounters || (now - lastCounterPlayAt) > 2500;
    ...
    const scheduleSettle = (delay = 80, options = {}) => {
        window.clearTimeout(settleTimer);
        settleTimer = window.setTimeout(() => settlePreview(options), delay);
    };
    ...
    editor.on('load', () => scheduleSettle(120, { forceCounters: true }));
    editor.on('canvas:frame:load', () => scheduleSettle(160, { forceCounters: true }));
    editor.on('voodbuilder:chrome-layout-ready', () => scheduleSettle(200, { forceCounters: true }));
    editor.on('voodbuilder:site-chrome-updated', () => scheduleSettle(400, { forceCounters: false }));
