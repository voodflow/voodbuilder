...
function buildValueControl(condition, options, labels) {
    ...
    wrap.className = 'voodbuilder-editor-condition-value';

    let control;

    if (meta.type === 'boolean') {
        control = document.createElement('select');
        control.className = 'voodbuilder-editor-input voodbuilder-editor-input--inline';
...
export function registerConditionsUi(editor, options = {}) {
    ...
    mount.innerHTML = `
        <div class="voodbuilder-editor-conditions voodbuilder-editor-conditions-panel">
            <p class="voodbuilder-editor-panel-subtitle">${labels.conditionsTitle ?? 'Visibility'}</p>
            <p class="voodbuilder-editor-hint">${labels.conditionsHint ?? ''}</p>
            <div data-voodbuilder-conditions-sets class="voodbuilder-editor-conditions-sets"></div>
            <p class="voodbuilder-editor-conditions-feedback" data-voodbuilder-conditions-feedback hidden></p>
            <div class="voodbuilder-editor-actions voodbuilder-editor-conditions-actions">
                <button type="button" class="voodbuilder-editor-btn" data-voodbuilder-conditions-add-set>
                    ${labels.conditionsAddSet ?? 'Add group'}
                </button>
                <button type="button" class="voodbuilder-editor-btn voodbuilder-editor-btn--ghost" data-voodbuilder-conditions-clear>
    ...
    const render = () => {
        ...
        const panelRoot = mount.querySelector('.voodbuilder-editor-conditions');
        const actions = mount.querySelector('.voodbuilder-editor-conditions-actions');
        const hint = mount.querySelector('.voodbuilder-editor-hint:not(.voodbuilder-editor-chrome-layout-notice)');
        const subtitle = mount.querySelector('.voodbuilder-editor-panel-subtitle');
        ...
        if (selectionNotice !== null) {
            ...
            if (panelRoot) {
                panelRoot.classList.add('voodbuilder-editor-conditions--chrome-layout-notice');
            }
        ...
        if (panelRoot) {
            panelRoot.classList.remove('voodbuilder-editor-conditions--chrome-layout-notice');
        }
        ...
        if (! hasSets) {
            setsMount.innerHTML = `
                <div class="voodbuilder-editor-conditions-empty">
                    <p class="voodbuilder-editor-hint">${labels.conditionsEmpty ?? 'No visibility rules. The element is always shown.'}</p>
                </div>
            `;
        ...
        definition.sets.forEach((set, setIndex) => {
            if (setIndex > 0) {
                ...
                divider.className = 'voodbuilder-editor-conditions-operator';
...
function buildSetBlock(definition, setIndex, set, options, labels, render, component, applyDefinition) {
    ...
    setEl.className = 'voodbuilder-editor-condition-card';

    const header = document.createElement('div');
    header.className = 'voodbuilder-editor-condition-card__header';

    const title = document.createElement('span');
    title.className = 'voodbuilder-editor-condition-card__title';
    ...
    removeSet.className = 'voodbuilder-editor-icon-btn';
    ...
    body.className = 'voodbuilder-editor-condition-card__body';
    ...
    conditions.forEach((condition, condIndex) => {
        if (condIndex > 0) {
            ...
            andBadge.className = 'voodbuilder-editor-conditions-join';
    ...
    addCond.type = 'button';
    addCond.className = 'voodbuilder-editor-btn voodbuilder-editor-btn--ghost voodbuilder-editor-btn--block';
...
function buildConditionRow(setIndex, condIndex, condition, options, labels, render, component, conditionCount, applyDefinition) {
    ...
    row.className = 'voodbuilder-editor-condition-item';

    const fields = document.createElement('div');
    fields.className = 'voodbuilder-editor-condition-item__fields';

    const keySelect = document.createElement('select');
    keySelect.className = 'voodbuilder-editor-select';
    ...
    compareSelect.className = 'voodbuilder-editor-select';
    ...
    valueSlot.className = 'voodbuilder-editor-condition-item__value';
    ...
    remove.type = 'button';
    remove.className = 'voodbuilder-editor-icon-btn';
    ...
    metaRow.className = 'voodbuilder-editor-condition-item__meta';
