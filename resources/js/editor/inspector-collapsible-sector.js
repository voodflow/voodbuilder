/**
 * Collapsible inspector sectors — same look as Editor Style Manager sectors.
 */

export function mountCollapsibleInspectorSector(mount, { title, open = true } = {}) {
    if (! mount || mount.dataset.voodbuilderSectorReady === '1') {
        return mount?.closest('.voodbuilder-editor-inspector-sector') ?? null;
    }
    ...
    sector.className = `voodbuilder-editor-inspector-sector${open ? ' is-open' : ''}`;

    const toggle = document.createElement('button');
    toggle.type = 'button';
    toggle.className = 'voodbuilder-editor-inspector-sector__title';
    toggle.setAttribute('aria-expanded', open ? 'true' : 'false');
    toggle.innerHTML = `<span class="voodbuilder-editor-inspector-sector__caret" aria-hidden="true"></span><span class="voodbuilder-editor-inspector-sector__label">${title ?? ''}</span>`;

    const body = document.createElement('div');
    body.className = 'voodbuilder-editor-inspector-sector__body';
