/**
 * Editor canvas component toolbar (parent select, drag, clone, delete) + dynamic bindings.
...
function buildDynamicToolbarButtons(labels = {}, { showMake = true, showClear = false } = {}) {
    ...
    if (showMake) {
        buttons.push({
            attributes: {
                class: 'voodbuilder-editor-toolbar-item--dynamic',
    ...
    if (showClear) {
        buttons.push({
            attributes: {
                class: 'voodbuilder-editor-toolbar-item--clear-dynamic',
...
function buildComponentToolbar(editor, component, labels = {}) {
    ...
    if (component.collection && hasSelectableParent(component, editor)) {
        toolbar.push({
            attributes: {
                class: 'voodbuilder-editor-toolbar-item--parent',
    ...
    if (canReorder && siblingIndex > 0) {
        toolbar.push({
            attributes: {
                class: 'voodbuilder-editor-toolbar-item--move-up',
    ...
    if (canReorder && siblingIndex < siblingCount - 1) {
        toolbar.push({
            attributes: {
                class: 'voodbuilder-editor-toolbar-item--move-down',
    ...
    if (component.get('copyable') && canDuplicateChromeEditorComponent(component, editor)) {
        toolbar.push({
            attributes: {
                class: 'voodbuilder-editor-toolbar-item--duplicate',
    ...
    if (editor.__voodbuilderImageEditorEnabled && resolveEditableImageTarget(component)) {
        toolbar.push({
            attributes: {
                class: 'voodbuilder-editor-toolbar-item--edit-image',
    ...
    if (! richText && canEditBlockCode(component, editor)) {
        ...
        toolbar.push({
            attributes: {
                class: 'voodbuilder-editor-toolbar-item--copy-code',
    ...
    toolbar.push({
        attributes: {
            class: 'voodbuilder-editor-toolbar-item--copy-classes',
...
/**
 * Editor plugin — register copy commands before project hydration.
 *
 * @param {object} editor
...
export function ensureCanvasComponentToolbarButtons(editor, component, labels = {}) {
    ...
    // Second pass after Editor positions the toolbar / canvas attrs settle on ?edit=1.
...
export function registerCanvasComponentToolbar(editor, labels = {}) {
    ...
    editor.on('load', () => {
        ensureCopyComponentCommands(editor, labels);
        // Re-apply content-width from data attrs (inline styles / private CSS may be missing after reload).
        const restore = () => {
            try {
