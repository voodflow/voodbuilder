/**
 * Layer tree reorder — custom pointer DnD (GrapesJS Layers sorter breaks after
...
function clearCanvasDropCues(editor) {
    const classes = [
        'voodbuilder-gjs-block-dragging',
        'voodbuilder-gjs-inner-drop-dragging',
        'voodbuilder-gjs-section-gap-drop',
    ];
...
function selectFromLayers(editor, component) {
    ...
    // GrapesJS LayerManager.componentChanged collapses every open layer that is
 ...
 * nest targets (e.g. select Hr → GrapesJS closes sibling Container/Div).
 *
 * @returns {object[]}
 */
...
function ensureInsertCue(mount) {
    ...
    if (cue) {
        ...
        cue.querySelector('.voodbuilder-gjs-layer-insert-label')?.remove();
    ...
    cue.className = 'voodbuilder-gjs-layer-insert-cue';
    cue.hidden = true;
    cue.innerHTML = `<div class="voodbuilder-gjs-layer-insert-line" ${LAYER_INSERT_LINE_ATTR}></div>`;
...
function clearDropScopeHighlight(mount) {
    mount.querySelectorAll('.voodbuilder-gjs-layer-drop-blocked').forEach((el) => {
        el.classList.remove('voodbuilder-gjs-layer-drop-blocked');
    });
}
...
function hideInsertLine(mount) {
    ...
    mount.querySelectorAll(
        '.voodbuilder-gjs-layer-drop-into, .voodbuilder-gjs-layer-drop-beside, .voodbuilder-gjs-layer-drop-blocked',
    ).forEach((el) => {
        el.classList.remove(
            'voodbuilder-gjs-layer-drop-into',
            'voodbuilder-gjs-layer-drop-beside',
            'voodbuilder-gjs-layer-drop-blocked',
        );
    });
}

/**
 * GrapesJS nests via padding-left on .gjs-layer-title — the title box itself
...
function markBlockedLayerAtPoint(mount, editor, source, clientX, clientY) {
    ...
    if (intoBlocked || besideBlocked || isSiteChromeBlock(target, editor) || isProtectedSlot(target)) {
        layerEl.classList.add('voodbuilder-gjs-layer-drop-blocked');
    }
}
...
function showInsertLine(mount, drop) {
    ...
    if (nestInto) {
        drop.layerEl.classList.add('voodbuilder-gjs-layer-drop-into');
        ...
        drop.layerEl.classList.add('voodbuilder-gjs-layer-drop-beside');
 ...
 *
 * `index` is the GrapesJS `move({ at })` value in *pre-removal* collection
 * coordinates (GrapesJS itself adjusts when moving down).
 */
function normalizeSiblingDrop(editor, source, parent, target, layerEl, placeBefore) {
    ...
    // GrapesJS Component.move no-ops when at === from || at === from + 1.
...
function applyLayerMove(source, drop) {
    ...
    if (sameParent) {
        ...
        // GrapesJS already compensates moving down (at > from → at-1) and treats
    ...
    // GrapesJS move() relocates the whole subtree (parent keeps all children).
...
function startCustomLayerReorder(editor, mount, component, event) {
    ...
    // Undo GrapesJS auto-collapse of non-ancestor layers (Container/Div, etc.).
    restoreOpenLayers(openSnapshot);

    const sourceLayerEl = getLayerRootElement(component);
    sourceLayerEl?.classList?.add('voodbuilder-gjs-layer-dragging');
    ...
    const onMove = (moveEvent) => {
        ...
        if (! started) {
            ...
            started = true;
            mount.classList.add('voodbuilder-gjs-layers-sorting');
            restoreOpenLayers(openSnapshot);
        }
    ...
    const cleanup = () => {
        ...
        sourceLayerEl?.classList?.remove('voodbuilder-gjs-layer-dragging');
        mount.classList.remove('voodbuilder-gjs-layers-sorting');
        hideInsertLine(mount);
...
function handleLayerMouseDown(editor, mount, event) {
    ...
    // Own the gesture — do not let GrapesJS ItemView.startSort run (broken sorter).
...
export function registerLayersDrag(editor, options = {}) {
    ...
    // Capture: stop GrapesJS native layer sorter before it starts.
