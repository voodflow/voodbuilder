...
component:selected
  ...
  → descriptor.render({ mount, root, editor }) // custom form in .voodbuilder-gjs-site-chrome-settings-mount
```
...
When a descriptor matches, `registerSettingsUi()` wraps `TraitManager.select()` so GrapesJS
