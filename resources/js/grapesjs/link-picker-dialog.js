...
export function linkPickerDialog(options = {}) {
    ...
    return new Promise((resolve) => {
        ...
        modal.className = 'voodbuilder-gjs-modal voodbuilder-gjs-dialog voodbuilder-gjs-link-picker-dialog';
        modal.setAttribute('role', 'presentation');

        const panel = document.createElement('div');
        panel.className = 'voodbuilder-gjs-modal__panel voodbuilder-gjs-dialog__panel';
        panel.setAttribute('role', 'dialog');
        panel.setAttribute('aria-modal', 'true');

        const backdrop = document.createElement('div');
        backdrop.className = 'voodbuilder-gjs-modal__backdrop';
        backdrop.dataset.voodbuilderDialogCancel = '';

        const head = document.createElement('header');
        head.className = 'voodbuilder-gjs-modal__head';
        head.innerHTML = `
            <h2 class="voodbuilder-gjs-modal__title">${escapeHtml(dialogTitle)}</h2>
            <button type="button" class="voodbuilder-gjs-modal__close" data-voodbuilder-dialog-cancel aria-label="${escapeAttr(dismissLabel)}">×</button>
        `;

        const body = document.createElement('div');
        body.className = 'voodbuilder-gjs-modal__body voodbuilder-gjs-dialog__body voodbuilder-gjs-form';

        if (options.message) {
            const message = document.createElement('p');
            message.className = 'voodbuilder-gjs-dialog__message';
        ...
        footer.className = 'voodbuilder-gjs-dialog__footer';

        if (allowRemove) {
            const removeBtn = document.createElement('button');
            removeBtn.type = 'button';
            removeBtn.className = 'voodbuilder-gjs-btn voodbuilder-gjs-btn--ghost';
        ...
        cancelBtn.className = 'voodbuilder-gjs-btn voodbuilder-gjs-btn--ghost';
        cancelBtn.dataset.voodbuilderDialogCancel = '';
        cancelBtn.textContent = dismissLabel;

        const confirmBtn = document.createElement('button');
        confirmBtn.type = 'button';
        confirmBtn.className = 'voodbuilder-gjs-btn voodbuilder-gjs-btn--primary';
